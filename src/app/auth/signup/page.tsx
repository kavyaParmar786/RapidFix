    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-6" style={{ background: 'var(--bg-base)' }}>
        <div className="w-full max-w-sm text-center">
          ...
          <p className="text-sm text-white/40 mb-2">We sent a verification link to</p>
          <p className="text-sm font-medium text-white mb-8">{email}</p>
          <p className="text-xs text-white/25 mb-6">Click the link to activate your account, then sign in.</p>
      ...
      <div className="flex flex-1 items-center justify-center px-6 py-12">
        <div className="w-full max-w-sm">
          ...
          <div className="mb-7">
            ...
            <p className="text-sm text-white/40">Join 50,000+ users on RapidFix</p>
          </div>

          {/* Role toggle */}
          <div className="flex rounded-lg border border-white/10 bg-white/[0.03] p-0.5 mb-5">
            {(['customer', 'worker'] as UserRole[]).map(r => (
              <button key={r} type="button" onClick={() => setRole(r)}
                className={cn(
                  ...
                  role === r ? 'bg-white text-black' : 'text-white/40 hover:text-white/70'
                )}>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <button type="submit" disabled={loading} className="btn-primary w-full mt-1 py-2.5">
              {loading
                ? <><div className="h-3.5 w-3.5 rounded-full border border-black/20 border-t-black animate-spin mr-2" />Creating account…</>
