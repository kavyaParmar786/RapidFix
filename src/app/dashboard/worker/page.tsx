    ...
    <>
      ...
      <div className="min-h-screen pt-14" style={{ background: 'var(--bg-base)' }}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
          ...
          {/* Header */}
          <div className="flex items-start sm:items-center justify-between gap-4 flex-wrap mb-8 pb-6 border-b border-white/[0.06]">
            <div>
              <h1 className="text-xl font-bold text-white mb-0.5">Worker Dashboard</h1>
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2">
              <div className="flex gap-1 rounded-lg border border-white/[0.08] bg-white/[0.03] p-0.5 mb-5">
                {[{ key: 'active', label: 'Active Jobs' }, { key: 'completed', label: 'Completed' }].map(t => (
                  <button key={t.key} onClick={() => setTab(t.key as 'active' | 'completed')}
                    className={`flex-1 rounded-md py-2 text-xs font-medium transition-all ${
                      tab === t.key ? 'bg-white text-black' : 'text-white/40 hover:text-white'
                    }`}>
                    {t.label}
                  </button>
                ))}
              ...
              <div className="space-y-3">
                {displayJobs.length === 0 ? (
                  <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] p-12 text-center">
                    <Briefcase size={20} className="mx-auto mb-3 text-white/20" />
            ...
            {/* Chats sidebar */}
            <div>
              <h2 className="text-sm font-semibold text-white mb-4">Active Chats</h2>
              <div className="space-y-2">
                {chats.length === 0 ? (
                  ...
                  <Link key={chat.id} href={`/chat/${chat.id}`}
                    className="flex items-center gap-3 rounded-xl border border-white/[0.07] bg-white/[0.02] p-3.5 hover:border-white/15 hover:bg-white/[0.04] transition-all">
                    <div className="h-8 w-8 rounded-full bg-white/10 flex items-center justify-center text-xs font-semibold text-white shrink-0">
                      {chat.customerName?.[0]?.toUpperCase()}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-white truncate">{chat.customerName}</p>
                      <p className="text-xs text-white/35 truncate">{chat.lastMessage || chat.jobTitle}</p>
                    </div>
                    {chat.lastMessageAt && (
                      <span className="text-[10px] text-white/25 shrink-0">{formatRelativeTime(chat.lastMessageAt)}</span>
                    )}
                  </Link>
                ))}
              </div>
