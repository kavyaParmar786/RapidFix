        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {steps.map((step, i) => (
            <div key={i} className="relative">
              ...
              <div className="glass-card p-6 relative z-10 h-full transition-all duration-300 hover:border-black/20 hover:-translate-y-1">
                {/* Number + icon */}
                <div className="flex items-start justify-between mb-5">
                  <div className={`h-12 w-12 rounded-2xl bg-gradient-to-br ${step.gradient} flex items-center justify-center shadow-lg`}>
                    <step.icon size={22} className="text-zinc-900" />
                  </div>
                  <span
                    className="text-4xl font-bold opacity-20 text-zinc-900"
