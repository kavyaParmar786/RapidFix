      ...
      <div className="flex h-screen flex-col pt-16" style={{ background: 'var(--bg-base)' }}>
        ...
        <div className="border-b border-white/10 px-4 py-3 flex items-center gap-3"
          ...
          <Link
            ...
            className="flex h-8 w-8 items-center justify-center rounded-lg border border-white/10 bg-white/5 text-white/70 hover:text-white transition-colors"
          >
            <ArrowLeft size={14} />
          </Link>
          ...
          <Link
            ...
            className="hidden sm:flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-white/60 hover:text-white transition-colors"
          >
            View Job
          </Link>
        </div>
          ...
          {grouped.map(({ date, msgs }) => (
            <div key={date}>
              ...
              <div className="flex items-center gap-3 my-4">
                ...
                <span className="text-xs text-white/30 bg-white/5 rounded-full px-3 py-1">{date}</span>
                <div className="flex-1 h-px bg-white/10" />
              </div>
                        ...
                        <div
                          className={cn(
                            ...
                            isMe
                              ...
                              : 'rounded-bl-sm bg-white/8 text-white border border-white/10'
                          )}
                        >
                          {msg.imageUrl && (
        ...
        {imagePreview && (
          <div className="px-4 py-2 border-t border-white/10 flex items-center gap-2"
            ...
            <p className="text-xs text-white/60 flex-1">{imageFile?.name}</p>
            <button onClick={() => { setImageFile(null); setImagePreview(null) }}
              className="h-6 w-6 flex items-center justify-center rounded-full bg-white/10 text-white/60 hover:bg-red-500/30 hover:text-red-400">
              <X size={12} />
            </button>
          </div>
        )}
        ...
        {/* Input bar */}
        <div className="border-t border-white/10 px-4 py-3 flex items-end gap-3"
          style={{ background: 'rgba(15,20,32,0.95)', backdropFilter: 'blur(20px)' }}>
          <input ref={fileRef} type="file" accept="image/*" onChange={handleImageChange} className="hidden" />
          <button
            onClick={() => fileRef.current?.click()}
            className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-white/60 transition-all hover:bg-white/10 hover:text-white"
          >
            <ImageIcon size={16} />
          </button>
          ...
          <button
            ...
            className={cn(
              ...
              sending || (!text.trim() && !imageFile)
                ...
                : 'bg-gradient-to-br from-indigo-500 to-violet-600 text-white hover:shadow-lg hover:shadow-indigo-500/30'
            )}
          >
            <Send size={16} />
          </button>
